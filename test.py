...
def run_pipeline(config, verbose=0):
    ...
    pairout = config["outputdir"] + "/" + config["output_dsigma"] + ".%03d"%(comm.rank)
    ...
    wpipe = weaklens.weaklens(rmin, rmax, rbin, pairout, True, config["Omegam"], config["outputpairfile"])
    wpipe.setup_random_rotate(comm.rank)
