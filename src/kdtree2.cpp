...
inline double squared(const double x, const double per) {
    double y=fabs(x);
  if(per>0.0){
    if(y>per/2.) y=per-y;
  }
...
// constructor
KDTree::KDTree(KDTreeArray& data_in,KDTreePeriod& xp,int xN, int xdim, int bs, bool rearrange_in,int dim_in)
  : the_data(data_in),
    period(xp),
    N  (xN ),
    dim(xdim),
    ...
    ind(N)
{
  ...
  if (rearrange) {
    ...
    rearranged_data=new double[N*dim];
    
    // permute the data for it.
    for (int i=0; i<N; i++) {
      for (int j=0; j<dim; j++) {
	rearranged_data[i*dim+j] = the_data[ind[i]*dim+j];
    ...
    // Let us do it in place
	for (int i=0; i<N; i++) {
	  for (int j=0; j<dim; j++) {
	    the_data[i*dim+j] = rearranged_data[i*dim+j];
	  }
	}
	delete []rearranged_data;
	data = &the_data;
    }
  } else {
    data = &the_data;
...
KDTreeNode* KDTree::build_tree_for_range(int l, int u, KDTreeNode* parent) {
  ...
  if ((u-l) <= bucketsize) {
    ...
    if (u-l>100000) {
      ...
      /// Use median finding routine here
      /*
      if(true){
	  /// Divide box[c].upper to box[c].lower into 1000 containers
	  int Nc=1000;
	  int *hist;
	  hist=new int[Nc];
	  for (int i=0;i<Nc;i++)hist[i]=0;
	  double diff=(node->box[c].upper-node->box[c].lower)/Nc;
	for (int k=l; k <= u; k++) {
	  int bin=int((the_data[ind[k]*dim+c]-node->box[c].lower)/diff);
	  if(bin>=0&&bin<Nc) hist[bin]++;
	}
	  // Find median
	  sum=0.0;
	  for(int i=0;i<Nc;i++){
	      sum=sum+hist[i];
	      if(sum/(u-l+1)>0.5){
	      average=node->box[c].lower+diff*(i+0.5);
	      break;}
	  }
	  delete []hist;
      }else */
	...
	for (int k=l; k <= u; k++) {
	  cn++;
	  sum += (the_data[ind[k]*dim+c]-sum)/cn;
	}
	average=sum;
	/*
	for (int k=l; k <= u; k++) {
...
void KDTree:: spread_in_coordinate(int c, int l, int u, interval& interv)
{
  ...
  if (i == u+1) {
    double last = the_data[ind[u]*dim+c];
...
void KDTree::n_nearest_brute_force(std::vector<double>& qv, int nn, KDTreeResultVector& result) {
  ...
  for (int i=0; i<N; i++) {
    ...
    KDTreeResult e; 
    for (int j=0; j<dim; j++) {
      dis += squared( the_data[i*dim+j] - qv[j], period[j]);
    }
    e.dis = dis;
    e.idx = i;
    result.push_back(e);
...
void KDTree::r_nearest_brute_force(std::vector<double>& qv, double rr, KDTreeResultVector& result) {
...
SearchRecord::SearchRecord(std::vector<double>& qv_in, KDTree& tree_in,
...
void KDTreeNode::print(){
    if(left==NULL&&right==NULL){
    	printf("%d %d %f %f %f %f %f %f \n",l,u,box[0].lower,box[1].lower,box[2].lower,box[0].upper,box[1].upper,box[2].upper);
    }
    if(left!=NULL)left->print();
    if(right!=NULL)right->print();
}

inline double dis_from_bnd(double x, double amin, double amax, double per) {
  if(per<0.0){
    if (x > amax) {
      return(x-amax); 
    } else if (x < amin)
      ...
      double dx1=fabs(x-amax);
      double dx2=fabs(x-amin);
      ...
      }else
      {
        if(false){
          ...
          double extra;
            ...
            double qval = sr.qv[cut_dim]; 
            double dl=dis_from_bnd(qval,left->box[cut_dim].lower,left->box[cut_dim].upper,sr.period[cut_dim]);
            double dr=dis_from_bnd(qval,right->box[cut_dim].lower,right->box[cut_dim].upper,sr.period[cut_dim]);
            if(fabs(dl)<fabs(dr)){
                ncloser=left;
                nfarther=right;
                extra=dr;
...
void KDTreeNode::process_terminal_node(SearchRecord& sr) {
  ...
  for (int i=l; i<=u;i++) {
    ...
    if (rearrange) {
      ...
      dis = 0.0;
      for (int k=0; k<dim; k++) {
	dis += squared(data[i*dim+k] - sr.qv[k],sr.period[k]);
	if (dis > ballsize) {
	  early_exit=true; 
	  break;
	}
      ...
      dis = 0.0;
      for (int k=0; k<dim; k++) {
	dis += squared(data[indexofi*dim+k] - sr.qv[k],sr.period[k]);
	if (dis > ballsize) {
	  early_exit= true; 
	  break;
	}
